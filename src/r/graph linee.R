...
m <- leaflet() %>%
  ...
  addCircles(data = data2, lng = ~B_lon, lat = ~B_lat, radius = (data2$occurences)^0.7,
             label = paste0(data2$B," - ",data2$A), fillColor =  "orange",  weight = 3, color = "blue") %>%
  addCircles(data = data2, lng = ~A_lon, lat = ~A_lat, radius = (data2$occurences)^0.7,
             label = paste0(data2$A," - ",data2$B), fillColor = "orange", weight =3, color = "blue")
